...
namespace Mandor.AI
{
    public class LongTermMemory
    {
        ...
        public async Task MakeMemory(string state)
        {
            ...
            try
            {
                mem = await OpenAI.GetChatCompletion(ChatModel.ChatGPT, new List<ChatMessage>() { tagsListMessage, Texts.ShortTermMemoryAddPrompt, stateMessage, Texts.ShortTermMemoryAddFormat });
            ...
            try
            {
                ...
                Logger.WriteLine("New short term memory:" + mem);
            }
            catch (Exception)
            {
                Logger.WriteLine("Add memory failed");
            }
        ...
        public async Task<string> GetMemories(string input)
        {
            ...
            while (true)
            {
                ...
                {
                    Logger.WriteLine($"Get memory failed - trying again: {ex}");
                }
