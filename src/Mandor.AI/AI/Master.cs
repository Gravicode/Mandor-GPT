    ...
    public class Master
    {
        ...
        public static Master Singleton 
        {
            get 
            {
                if (singleton == null)
                {
                    Logger.WriteLine("Create master");
                    singleton = new Master();
                    Logger.WriteLine("Master created");
                }
        ...
        public async Task Tick()
        {
            ...
            Logger.WriteLine("master tick -master");
            ...
            if (Memory.memories.Count == 0)
            {
                ...
                Logger.WriteLine("master start memory done");
            }
            ...
            while (true)
            {
                ...
                {
                    Logger.WriteLine($"Master failed - trying again: {ex}");
                }
            }
            ...
            Logger.WriteLine(JsonSerializer.Serialize(masterInput, new JsonSerializerOptions() { WriteIndented = true }));
            Logger.WriteLine(scriptFile.GetText());
            Logger.WriteLine("------------------------------------------------------------------------");
